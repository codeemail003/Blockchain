...
class PharmaChain extends EventEmitter {
  ...
  checkEnvironmentalViolations(batch) {
    ...
    // Check humidity
    if (
      latestHumidity.value < minHumidity ||
      latestHumidity.value > maxHumidity
    ) {
      this.recordComplianceViolation({
        ...
        threshold:
          latestHumidity.value < minHumidity ? minHumidity : maxHumidity,
  ...
  recordComplianceViolation(violation) {
    ...
    this.metrics.complianceRate =
      ((totalChecks - violationCount) / totalChecks) * 100;
  ...
  getMetrics() {
    return {
      ...
      activeRecalls: Array.from(this.recallRegistry.values()).filter(
        (recall) => recall.status === "ACTIVE"
      ).length,
