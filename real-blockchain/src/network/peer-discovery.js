...
class PeerDiscovery extends EventEmitter {
    constructor(options = {}) {
        ...
        console.log(`Peer Discovery initialized for node ${this.nodeId} on ${this.networkType}`);
    }
    ...
    async connectToBootstrapNodes() {
        const connectionPromises = this.bootstrapNodes.map(async (nodeAddress) => {
            try {
                ...
                console.warn(`Failed to connect to bootstrap node ${nodeAddress}:`, error.message);
            }
        });
    ...
    async validatePeerCompatibility(peerInfo) {
        ...
        if (!acceptableCompliance.includes(peerInfo.complianceLevel)) {
            console.warn('Compliance level not acceptable:', peerInfo.complianceLevel);
            return false;
        }
        
        // Check geographical restrictions if applicable
        if (this.geographicalRegion !== 'GLOBAL' && 
            peerInfo.geographicalRegion !== this.geographicalRegion &&
            peerInfo.geographicalRegion !== 'GLOBAL') {
            console.warn('Geographical region restriction:', peerInfo.geographicalRegion);
            return false;
        }
        ...
        try {
            ...
            switch (message.type) {
                ...
                default:
                    ...
                    this.emit('peerMessage', { message, peer: this.findPeerBySocket(ws) });
            }
