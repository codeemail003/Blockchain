...
function initializeCharts() {
  ...
  const batchCtx = document
    .getElementById("batch-performance-chart")
    .getContext("2d");
  new Chart(batchCtx, {
    type: "line",
    data: {
...
function setupEventHandlers() {
  // Create Batch Form
  document
    .getElementById("create-batch-form")
  ...
  // Verify Batch Form
  document
    .getElementById("verify-batch-form")
    .addEventListener("submit", async function (e) {
      e.preventDefault();
      try {
...
function updateMetrics(metrics) {
  ...
  document.getElementById(
    "compliance-rate"
  ).textContent = `${metrics.complianceRate}%`;
  document.getElementById("cold-chain-status").textContent =
    metrics.coldChainStatus;
...
function updateBatchTable(batches) {
  ...
  batches.forEach((batch) => {
    ...
    row.innerHTML = `
            ...
            <td><span class="badge bg-${getBatchStatusColor(batch.status)}">${
      batch.status
    }</span></td>
            <td>${batch.temperature}°C</td>
            <td>
                <button class="btn btn-sm btn-primary" onclick="viewBatchDetails('${
                  batch.id
                }')">
                    <i class="ri-eye-line"></i>
                </button>
                <button class="btn btn-sm btn-success" onclick="verifyBatch('${
                  batch.id
                }')">
...
function updateComplianceTable(compliance) {
  ...
  compliance.forEach((report) => {
    ...
    row.innerHTML = `
            ...
            <td><span class="badge bg-${getComplianceStatusColor(
              report.status
